...
*Setup instructions for a gitian build of SEND using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the SEND
...
Clone the git repositories for send and gitian and then checkout the send version that you want to build.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/send-crypto/send.git
cd send
...
Follow the instructions in [doc/release-process.md](release-process.md) in the send repository
...
To build SEND (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the send repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/send/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/send-crypto/send
...
For example:
```bash
URL=https://github.com/crowning-/send.git
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/send-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/send-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/send-build.assert
```
...
[send/gitian.sigs](https://github.com/send-crypto/gitian.sigs/) repository, or if that's not possible to create a pull
request.
