...
*Setup instructions for a Gitian build of SEND Core using a VM or physical system.*
...
- [Building Send Core](#building-send-core)
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/gitianuser/gitian-builder/inputs/send/.git/
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/send-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/send-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/send-osx-build.assert
```
...
[send-crypto/gitian.sigs](https://github.com/send-crypto/gitian.sigs/) repository, or if that's not possible create a pull
